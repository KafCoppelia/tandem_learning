...
class LinearBN1d_if(nn.Module):
    ...
    def forward(self, input_feature_st, input_features_sc):
        ...
        weightNorm = torch.mul(
            linearif_weight.permute(1, 0), ratio).permute(1, 0)
        ...
        # spike train
        output_st, output_sc = self.linearif(
            input_feature_st, output, weightNorm, self.device, biasNorm, self.neuronParam)

        return output_st, output_sc


class LinearBN1dInference_if(nn.Module):
    """Compound layer with IF neuron: Linear + BN"""

    def __init__(self, weightNorm, biasNorm, device=torch.device('cuda'), bias=True, neuronParam=None):
        super(LinearBN1dInference_if, self).__init__()
        self.linearif = LinearIF.apply
        self.device = device
    ...
    def forward(self, input_feature_st):
        output_st, _ = self.linearif(
            input_feature_st, 0, self.weightNorm, self.device, self.biasNorm, self.neuronParam)
...
class ConvBN2d_if(nn.Module):
    ...
    def __init__(self, Cin, Cout, kernel_size, device=torch.device('cuda'), stride=1,
        ...
        self.conv2d = torch.nn.Conv2d(
            Cin, Cout, kernel_size, stride, padding, bias=bias)
...
class ConvBN2dInference_if(nn.Module):
    """Compound layer with IF neuron: Conv2d + BN"""

    def __init__(self, weightNorm, biasNorm, device=torch.device('cuda'), stride=1,
                 padding=0, neuronParam=None):

        super(ConvBN2dInference_if, self).__init__()
        self.conv2dIF = Conv2dIF.apply
        self.weightNorm = weightNorm
        self.biasNorm = biasNorm
        self.device = device
    ...
    def forward(self, input_feature_st):
        output_features_st, _ = self.conv2dIF(input_feature_st, 0, self.weightNorm, self.device, self.biasNorm, self.stride,
                                              self.padding, self.neuronParam)
